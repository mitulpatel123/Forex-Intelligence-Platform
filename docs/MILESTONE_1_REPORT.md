...
**MILESTONE 1 LIVE IC MARKETS EUR/USD VERTICAL SLICE PASSED**
...
- Narrow MV3 browser bridge with exact origin allowlists, iframe injection,
  observer-ready state, bounded text/binary candidates, token redaction, and the
...
- The visible DOM path depends on the terminal continuing to expose an EURUSD row
  with two visible decimal values.
