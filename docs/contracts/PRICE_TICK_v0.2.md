...
- visible-DOM `provider_event_time` and provider `sequence` remain null; they are
  never inferred.
- `browser_observed_at` is the extension observation time. It is not provider time.
- for `DISPLAY_QUOTE`, compatibility field `received_at` means
  `browser_observed_at`.
- `collector_received_at` is generated by the collector, and `normalized_at` is
  generated after validation.
- `observation_sequence` is strictly increasing per
  `document_session_id + instrument` and resets only with a new document session.
- the three local delay fields measure browser-to-collector, collector processing,
  and the total browser-to-normalization path in milliseconds.
